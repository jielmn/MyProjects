...
void CRingUI::SetBkImage( LPCTSTR pStrImage )
{
	...
	if (m_diBk.sImageName == pStrImage) return;
	m_diBk.sImageName = pStrImage;
	DeleteImage();
	Invalidate();
}
...
{
	m_pBkimage = CRenderEngine::GdiplusLoadImage(m_diBk.sImageName);
	if ( NULL == m_pBkimage ) return;
	if(m_pManager) m_pManager->SetTimer(this, RING_TIMERID, 200);
}
