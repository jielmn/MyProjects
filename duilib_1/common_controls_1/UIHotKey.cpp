...
void CHotKeyUI::PaintStatusImage(HDC hDC)
{
	...
	if( (m_uButtonState & UISTATE_DISABLED) != 0 ) {
		if( !m_sDisabledImage.IsEmpty() ) {
			tDraw.Clear();
			tDraw.sDrawString = m_sDisabledImage;
			if( !DrawImage(hDC, tDraw) ) {}
			else return;
		}
	}
		...
		if( !m_sFocusedImage.IsEmpty() ) {
			tDraw.Clear();
			tDraw.sDrawString = m_sFocusedImage;
			if( !DrawImage(hDC, tDraw) ) {}
			else return;
		}
	}
	...
	if( !m_sNormalImage.IsEmpty() ) {
		tDraw.Clear();
		tDraw.sDrawString = m_sNormalImage;
		if( !DrawImage(hDC, tDraw) ) {}
		else return;
	}
}
