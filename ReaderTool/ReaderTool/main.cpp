...
void  CDuiFrameWnd::OnSetReaderBlueToothNameRetMsg(int ret) {
	...
	if (0 == ret) {
		MessageBox(GetHWND(), "设置Reader 蓝牙名称成功", "设置Reader 蓝牙名称", 0);
	}
	else {
		MessageBox(GetHWND(), GetErrorDescription(ret), "设置Reader 蓝牙名称", 0);
	}
}
...
void  CDuiFrameWnd::OnSetBlueToothName() {
	...
	if (strText.GetLength() == 0) {
		::MessageBox(GetHWND(), "Reader 蓝牙名称不能为空", "设置Reader 蓝牙名称", 0);
		return;
	}
