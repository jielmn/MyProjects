...
void CGraphicsRoundRectPath::AddRoundRect(INT x, INT y, INT width, INT height, INT cornerX, INT cornerY)
{
	...
	AddArc(x, y, elWid, elHei, 180, 90); // ×óÉÏ½ÇÔ²»¡
	AddLine(x + cornerX, y, x + width - cornerX, y); // ÉÏ±ß

	AddArc(x + width - elWid, y, elWid, elHei, 270, 90); // ÓÒÉÏ½ÇÔ²»¡
	AddLine(x + width, y + cornerY, x + width, y + height - cornerY);// ÓÒ±ß

	AddArc(x + width - elWid, y + height - elHei, elWid, elHei, 0, 90); // ÓÒÏÂ½ÇÔ²»¡
																		//AddLine(x + width - cornerX, y + height, x + cornerX, y + height); // ÏÂ±ß
