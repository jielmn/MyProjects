...
Page({
  ...
  getUsers: function () {
    ...
    wx.request({
      url: app.globalData.serverAddr + '?type=users',
      method: 'GET',
      success: (res) => {
  ...
  getTodolist : function (is_complete) {
    ...
    wx.request({
      url: app.globalData.serverAddr + '?type=any_todolist&user_id=' + encodeURIComponent(user_id) + '&is_complete=' + is_complete + '&start_index='+start_index,
      method: 'GET',
      success: (res) => {
        if (res.data.error != null && res.data.error == 0) {
